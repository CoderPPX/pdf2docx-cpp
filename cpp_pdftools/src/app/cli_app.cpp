  ...
  try {
    ...
    if (command == "text" && args.size() >= 3 && args[2] == "extract") {
      ...
      request.output_format =
          HasFlag(args, "--json") ? pdf::TextOutputFormat::kJson : pdf::TextOutputFormat::kPlainText;
    ...
    if (command == "image2pdf") {
      ...
      request.scale_mode =
          HasFlag(args, "--original-size") ? pdf::ImageScaleMode::kOriginal : pdf::ImageScaleMode::kFit;
    ...
    const Status status =
        CurrentExceptionToStatus(ErrorCode::kInternalError, "unexpected cli exception", "RunCli");
    return PrintStatus(err, status);
  }
}
