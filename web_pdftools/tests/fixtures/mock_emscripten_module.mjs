...
export default async function createMockEmscriptenModule() {
  ...
  const FS = {
    ...
    writeFile(filePath, data) {
      ...
      fsFiles.set(normalized, toUint8Array(data));
    },
    readFile(filePath) {
      const normalized = normalizePath(filePath);
  ...
  function _pdftools_wasm_op(requestPtr, requestLen, responsePtrPtr, responseLenPtr) {
    ...
    if (type === 'return-non-zero') {
      return 7;
    }

    if (type === 'bad-json') {
      writeResponseBytes(encoder.encode('not-json'), responsePtrPtr, responseLenPtr);
      return 0;
    }
